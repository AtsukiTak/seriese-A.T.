...
impl<const MAX: usize> AsRef<[u8]> for BytesAtMost<MAX> {
    fn as_ref(&self) -> &[u8] {
        &self.bytes[0..self.len]
    }
}

impl<const MAX: usize> AsMut<[u8]> for BytesAtMost<MAX> {
...
impl<const N: usize, const MAX: usize> From<[u8; N]> for BytesAtMost<MAX> {
    fn from(array: [u8; N]) -> Self {
        ...
        bytes.as_mut().copy_from_slice(&array[..]);
        bytes
    }
}
...
impl<const MAX: usize> From<u16> for BytesAtMost<MAX> {
    fn from(n: u16) -> Self {
        ...
        bytes.as_mut().write_u16::<LE>(n).unwrap();
        bytes
    }
}
...
impl<const MAX: usize> From<u32> for BytesAtMost<MAX> {
    fn from(n: u32) -> Self {
        ...
        bytes.as_mut().write_u32::<LE>(n).unwrap();
        bytes
    }
}
...
impl<const MAX: usize> From<u64> for BytesAtMost<MAX> {
    fn from(n: u64) -> Self {
        ...
        bytes.as_mut().write_u64::<LE>(n).unwrap();
        bytes
    }
}
